...
//運動部位選択
let pointofstretch = UITextField(frame: CGRect(x: (w2 - 300) / 2, y: 150, width: 300, height: 100))
//スケジュール内容入力テキスト
let eventText = UITextView(frame: CGRect(x: (w2 - 300) / 2, y: 200, width: 300, height: 100))
...
class EventViewController: UIViewController{
    ...
    let list: [String] = ["eyes", "neck", "shoulder", "Back" ]
...
//        let doneItem = UIBarButtonItem(barButtonSystemItem: .done, target: self, action: #selector(done))
        toolbar.setItems([spaceItem], animated: true)
        
        pointofstretch.inputView = pickerView
        pointofstretch.inputAccessoryView = toolbar
        ...
        eventInsert.setTitle("Add", for: UIControl.State())
        ...
        //「戻る!」ボタン
        let backBtn = UIButton(frame: CGRect(x: (w - 200) / 2, y: 700, width: 200, height: 50))
